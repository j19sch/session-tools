...
if __name__ == '__main__':
    ...
    session_overview, session_numbers, bugs, issues, questions = session_parser(args.files)

    with MarkDownWriter(f"analysis-{datetime.now().strftime('%Y%m%dT%H%M%S')}.md") as md_writer:
        ...
        overview_1_columns = [('', col_width_narrow, 'L'),
        ...
        overview_1_data = [('totals', len(session_overview), sum([int(_['duration']) for _ in session_numbers]),
                            sum([int(_['actual_duration']) for _ in session_numbers]),
                            len(bugs), len(issues), len(questions))]
        md_writer.table(overview_1_columns, overview_1_data)

        # Overview - charters
        overview_2_columns = [('File', col_width_default, 'L'), ('Tester', col_width_narrow, 'L'),
                              ('Charter', col_width_wide, 'L')]
        overview_2_data = [(session['session_notes'], session['tester'], session['charter'])
        ...
        numbers_columns = [('File', col_width_default, 'L'), ('planned (min)', col_width_narrow, 'C'),
                           ('spent (min)', col_width_narrow, 'C'), ('setup (%)', col_width_narrow, 'C'),
                           ('testing (%)', col_width_narrow, 'C'), ('investigating (%)', col_width_narrow, 'C')]
        numbers_data = [(session['session_notes'], session['duration'], session['actual_duration'],
                         session['setup'], session['testing'], session['investigating']) for session in session_numbers]

        separator = (' -   -   -', ' -   -   -', ' -   -   -', ' -   -   -', ' -   -   -', ' -   -   -')
        # ToDo: construct separator based on len(number_of_columns)
        numbers_data.append(separator)

        means = ('mean', round(mean([int(_['duration']) for _ in session_numbers])),
        ...
        medians = ('median', round(median([int(_['duration']) for _ in session_numbers])),
        ...
        if len(bugs) > 0:
            bugs_columns = [('File', col_width_default, 'L'), ('Bug', col_width_wide, 'L')]
        ...
        if len(issues) > 0:
            issues_columns = [('File', col_width_default, 'L'), ('Issue', col_width_wide, 'L')]
        ...
        if len(questions) > 0:
            questions_columns = [('File', col_width_default, 'L'), ('Question', col_width_wide, 'L')]
            questions_data = [(question['session_notes'], question['question']) for question in questions]
